...
export default function DateReserve({locationId, venuesJson, onDateChange, onLocationChange}:{locationId:string, venuesJson: VenueJson, onDateChange:Function, onLocationChange:Function}) {

    const [reserveDate, setReserveDate] = useState<Dayjs | null>(null);
    const [location, setLocation] = useState<string>('');

    useEffect(() => {
    const found = venuesJson.data.find(v => v._id === locationId);
        if (found) {
            setLocation(found._id);
            onLocationChange(found._id);
        }
    }, [locationId, venuesJson]);
